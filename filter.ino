...
const float FC[10] = {  2*1000/fs   2*2000/fs  2*3000/fs  2*4000/fs   2*5000/fs  2*6000/fs  *7000/fs  2*8000/fs   2*9000/fs  2*10000/fs  }; 
float x;
float y[10][]; //matrix of output values for each filter
...
void loop() {
  ...
  y[0][writeInd] = filter(&x, XBuff, &writeind, FC[0]); //FC[index] is the frequency that you want to filter for, the fractions correspond to [1000 Hz 2000 Hz....10000 Hz]
}

float filter(float *x, static float *XBuff, int *writeInd, float fc){
  float out = 0; //output for 1 time 
  *XBuff = *x;
  
  for(int convInd=0, convInd<N, convInd++){ //perform the convolution
    out += 2 * P[(N-convInd)%N)] * XBuff[(convInd+writeInd)%N] * cos(PI*fc*((N-convInd)%N)); //weird indexing on P is because of how circular buffer is constructed s.t. [... x[-1] x[0] x[-99] ...]
  }
